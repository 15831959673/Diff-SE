...
def train(file, i=None):
    logging.basicConfig(
        filename=f'integrative/{file}_result.log',
        filemode='w',
    ...
    F = ['mESC', 'myotube', 'macrophage',  'proB-cell', 'Th-cell']
    ...
    for k in range(10):
        ...
        Y_train = []
        for f in F:
            df = pd.read_csv(f'{f}_train{k+1}.csv')
            # 分离特征和标签
        ...
        df = pd.read_csv(f'{file}_test{k + 1}.csv')
        # 分离特征和标签
...
def test(file):
    ...
    for k in range(10):
        ...
        state_dict = torch.load(f'diff_contrastive_model/{file}/model{k}.pth')
