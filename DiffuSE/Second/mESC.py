...
def train(file, i=None):
    logging.basicConfig(
        filename=f'diffmodel/{file}/result.log',
        filemode='w',
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        criterion = ContrastiveLoss1().to(device)
        ...
        for epoch in range(epochs):
            ...
            loss1_epoch.append(np.mean(loss1_ls))
            loss2_3_epoch.append(np.mean(loss2_3_ls))
        ...
        loss_df = pd.DataFrame({
            ...
            'Loss1': loss1_epoch,
            'Loss2_3': loss2_3_epoch
        })
        loss_df.to_csv(f'diffmodel/{file}/{count}-fold_loss.csv', index=False)
...
def test(file):
    ...
    for train_index, test_index in skf.split(X, Y):
        ...
        model = Predictor().to(device)
        # state_dict = torch.load(f'diffmodel/{file}/model{k}.pth')
        state_dict = torch.load(f'diff_contrastive_model/mESC/model{k}.pth')
        ...
        with torch.no_grad():
            ...
            for x, y in tqdm(test_iter):
                ...
                y_pred_batch = (y_score > 0.5).float()
                y_pred = torch.cat((y_pred, y_pred_batch), dim=0)
            accuracy, precision, recall, f1, roc_auc, aupr, mcc = evaluate(y_pred.to('cpu'), y_scores.to('cpu'),
                                                                           y_true.to('cpu'))
            print(f"-----------k:{k}-----------")
            print(f"测试集的ACC为{accuracy * 100:.2f}%")
            print(f"测试集的PRE为{precision * 100:.2f}%")
        ...
        for train_index, test_index in skf.split(X, Y):
            ...
            for epoch in range(epochs):
                ...
                with torch.no_grad():
                    ...
                    print(f"-----------epoch:{epoch + 1}-----------")
                    print(
                        f"epoch: {epoch + 1}, loss: {np.mean(loss_ls):.5f}, loss1: {np.mean(loss1_ls):.5f}, loss2_3: {np.mean(loss2_3_ls):.5f}")
                    print(f"测试集的ACC为{accuracy * 100:.2f}%")
                    print(f"测试集的PRE为{precision * 100:.2f}%")
                    print(f"测试集的RECALL为{recall * 100:.2f}%")
                    print(f"测试集的F1为{f1 * 100:.2f}%")
